...
const Card2 = ({ page, handleNext, senior, handlePrev }) => {
	return (
		<div className={`perspective-2 ${page > 2 && "w-0 h-0"} absolute`}>
			<div
				className={` transition-all duration-1000 origin-left ${page > 2 && "rotate-y-135"} ${
					page == 2 && "rotate-y-20"
					...
					<>
						<div className="pl-5 pr-3 relative mb-2">
							<div className="flex flex-col">
								<div className="text-md text-[#fc618a] font-sans space-x-1">
									The results are just in! The COPS Award in category
								</div>
								<div className="text-[#fd7196] text-4xl space-x-3 font-bold font-sans mb-1 flex content-center">
									{senior.custom_award_title}
								</div>
								<span className="text-[#fd7196] text-lg">
									goes to none other than{" "}
									<div className="font-semibold text-3xl">{senior.name}</div>
								</span>
							</div>
						</div>
