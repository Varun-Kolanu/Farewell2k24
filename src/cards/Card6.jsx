...
const Card6 = ({ page, handleNext, handlePrev }) => {
	return (
		<div
			className={`perspective-6 ${page > 6 && "w-0 h-0"} absolute cursor-default`}
			onClick={() => page <= 6 && handleNext()}>
			<div
				className={` transition-all duration-1000 origin-left ${page > 6 && "rotate-y-135"} ${
					page == 6 && "rotate-y-20"
                  ...
                  text-4xl space-x-3 font-bold font-sans mb-1">
						...
						<div className="w-full px-3 flex flex-col items-center justify-between relative -top-8">
							<div className="relative">
								...
								<img src="/black_and_white.jpg" className="w-full h-full mb-2 z-10" />
							</div>
						</div>
					</>
				)}
